...
<div class="max-w-5xl mx-auto">
    ...
    @if($profile->status === 'rejected')
        <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 rounded-xl p-6 mb-8">
            <div class="flex gap-4">
                ...
                <div>
                    <h3 class="font-semibold text-red-800 dark:text-red-200 mb-2">Votre profil a été rejeté</h3>
                    <p class="text-red-700 dark:text-red-300 text-sm mb-3">{{ $profile->motif_rejet }}</p>
                    <p class="text-sm text-red-600 dark:text-red-400">Veuillez corriger les points mentionnés ci-dessus et resoummettre votre profil.</p>
                </div>
            </div>
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('operator.profile.update') }}" enctype="multipart/form-data"
        ...
        <div class="glass-card rounded-2xl p-8 border border-white/10">
            ...
            <div class="mt-6">
                ...
                <textarea name="bio" rows="4"
                          class="w-full px-4 py-3 bg-white dark:bg-dark-700 border border-gray-300 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-accent-blue focus:border-transparent transition-all text-gray-900 dark:text-white">{{ old('bio', $profile->bio) }}</textarea>
            </div>
        ...
        <div class="glass-card rounded-2xl p-8 border border-white/10">
            ...
            @if($profile->documents->count() > 0)
                <div class="mb-8">
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-4">Vos documents actuels</h3>
                    <div class="space-y-3">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 text-xs font-semibold rounded">Optionnel</span>
                </div>
                <p class="text-xs text-gray-600 dark:text-gray-400 mb-3">Laissez vide pour conserver votre CV actuel</p>
            ...
            <div>
                ...
                <p class="text-xs text-gray-600 dark:text-gray-400 mb-3">Certificats, diplômes, lettres de recommandation, etc.</p>
                <div class="relative border-2 border-dashed border-gray-300 dark:border-white/10 rounded-xl p-8 text-center hover:border-accent-blue transition-colors cursor-pointer group">
