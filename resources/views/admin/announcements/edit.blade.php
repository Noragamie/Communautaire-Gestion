...
@section('title', 'Modifier — ' . $announcement->title)
    ...
    <form method="POST" action="{{ route('admin.announcements.update', $announcement) }}" enctype="multipart/form-data"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 space-y-6">
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $announcement->title) }}" required autofocus
                       class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all @error('title') border-red-400 @enderror">
            ...
            <div>
                ...
                @if($announcement->image && !old('title'))
                    <div class="mb-3">
