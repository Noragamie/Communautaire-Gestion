...
@section('content')
    <div x-data="{ showRejectModal: false, rejectReason: '' }">
        <!-- Back Button -->
        <a href="{{ route('admin.profiles.index') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-primary-600 mb-6 font-medium transition-colors">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <!-- Header -->
            <div class="bg-gradient-to-r from-primary-500 to-primary-600 p-8">
            ...
            <div class="p-8">
                ...
                @if($profile->documents->count() > 0)
                    <div class="mt-8 pt-8 border-t border-gray-200">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="font-semibold text-gray-900 text-lg">Documents</h3>
                            <form method="POST" action="{{ route('admin.profiles.export-documents', $profile) }}" class="inline">
                                @csrf
                                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg font-medium transition-colors text-sm">
                ...
                @elseif($profile->status === 'rejected')
                    <div class="mt-8 pt-8 border-t border-gray-200">
                        <p class="text-sm text-gray-600 mb-4">L'utilisateur peut resoummettre son profil après correction.</p>
        ...
        <div x-show="showRejectModal"
            ...
            <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 border border-gray-200">
                <h3 class="text-2xl font-bold mb-4 text-gray-900">Rejeter le profil</h3>
                <p class="text-sm text-gray-600 mb-6">Veuillez expliquer la raison du rejet. L'utilisateur recevra ce message et pourra resoummettre son profil.</p>

                <form method="POST" action="{{ route('admin.profiles.reject', $profile) }}">
                    @csrf
                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-gray-900 mb-2">Motif du rejet *</label>
                        <textarea name="motif_rejet" x-model="rejectReason" required rows="4"
                                  class="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent text-gray-900"
                                  placeholder="Expliquez précisément les raisons du rejet..."></textarea>
                    </div>
                    <div class="flex gap-4">
                        <button type="button" @click="showRejectModal = false"
                                class="flex-1 px-6 py-3 border border-gray-300 rounded-xl hover:b
